...
class WordClouds(commands.Cog):
    ...
    @checks.is_owner()
    @commands.command()
    async def i(self, ctx, num: int=1):
        ...
        try:
            async for message in channel.history(limit=limit):
                if not message.author.bot:
                    if ctx.one or user == message.author:
                        text += message.clean_content + " "
            text = URL_RE.sub("", text)
        except discord.errors.Forbidden:
            await ctx.send("Wordcloud creation failed. I can't see that channel!")
            return
        command_list = []
        messages = ctx.channel.history(limit=20)
       
        for i in range(len(messages)):
            if messages[i][0] == '<':
                command_list.append(messages[i])
            if len(command_list) >= num:
                await ctx.send( str(messages[i]))

...
class RPG(commands.Cog):
    ...
    async def _duels_list(self, ctx, top: int = 10):
        ...
        names = {uid: server.get_member(uid) for uid, m in members.items()}
    ...
    async def _duel(self, ctx, user: discord.Member):
        ...
        p1_level, experience = await self.get_member_level(author)
        p2_level, experience = await self.get_member_level(user)
        p1 = Player(self, author, p1_items, 2 * p1_level + guild_config['initial_hp'])
        p2 = Player(self, user, p2_items, 2 * p2_level + guild_config['initial_hp'])
        ...
        try:
            ...
            msg = f"{p1} challenges {p2} to a duel!"
            msg += f"{p1} HP is {p1.hp}, {p2} HP is {p2.hp}\n"
            ...
            await ctx.send(msg)
            equipped = await self.get_equipped_slots(user)
            msg = f"{user.display_name}'s equipped items:\n{EQUIPPED.format(w = equipped['weapon'], h = equipped['helmet'], a = equipped['body_armor'], p = equipped['pants'], g = equipped['gloves'], b = equipped['boots'], s = equipped['shoulders'], heal = equipped['healing_item'])}"
            await ctx.send(msg)
            ...
            if p1.hp != p2.hp:
                ...
                await bank.deposit_credits(loser.member, 500)
                

                winnerexp = random.randint(5, 5 + victor.hp)
                loserexp = random.randint(0, 5 + victor.hp)
               
                member = victor.member
                await self.add_experience(member, int(winnerexp), any)
                member = loser.member
                await self.add_experience(member, int(loserexp), any)

                msg = f'\n.\n.\n.\nAfter **{i + 1} rounds**, {victor.mention} wins with **{victor.hp}** HP! \nThey have been awarded with **$2500 {currency}** and **{winnerexp} exp**!\n'
                msg += f'For sucking {loser.mention} was awarded with **500 {currency}** and **{loserexp} exp**.\n'
                msg += '\n.\n**SŦ𝙰ŦS:**\n.\n'
                msg += f'{p1} has **{await p1.get_wins()}** wins, **{await p1.get_losses()}** losses, **{await p1.get_draws()}** draws.\n'
                msg += f'{p2} has **{await p2.get_wins()}** wins, **{await p2.get_losses()}** losses, **{await p2.get_draws()}** draws.'
            else:
                victor = None
    ...
    @commands.guild_only()
    @commands.group(name="shoppe", invoke_without_command=True)
    async def _shoppe(self, ctx, category: str = None):
        """
        Item purchasing/selling
        """

        if ctx.invoked_subcommand is None:
            await ctx.invoke(self._shoppe_list, category)
            return

        await ctx.send_help()


    @_shoppe.command(name="list")
    async def _shoppe_list(self, ctx, category: str = None):
        """
        ...
        if category != None:
            ...
            final_msg = ''
            items = self.to_shoppe_items(items[category], category)
            table_rows = []
            for item in sorted(items, key=sort_cost):
                table_rows.append(self.to_shoppe_row(item, category))
        ...
        for category in items.keys():
            items[category] = self.to_shoppe_items(items[category], category)
            table_rows = []
    ...
    @_shoppe.command(name="buy")
    async def _shoppe_buy(self, ctx, *, item_name):
        """
    ...
    @_shoppe.command(name="sell")
    async def _shoppe_sell(self, ctx, *, item_name):
        """
    ...
    async def _addexp(self, ctx, amount: int, member: discord.Member = None):
        ...
        await ctx.send(f"Adding {amount} experience to {member.mention}!")
...
# UTILS BEGIN
    ...
    def to_shoppe_items(self, items, category):
        result = []
        if category == 'weapon':
            for item in items:
    ...
    def to_shoppe_row(self, item, category):
        if category == 'weapon':
